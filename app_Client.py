...
def charger_projet_client(tab_chargement):
    with tab_chargement:
        ...
        fichier_json = st.file_uploader(
            "Importer votre fichier projet client",
            type=["json"]
        )
        ...
        if "production_consommation_horaire" not in projet_client:
            st.error("Le fichier JSON importé n'est pas au bon format client.")
            st.write("Clés trouvées dans le fichier :", list(projet_client.keys()))
            return None
        ...
        st.success("Configuration client chargée.")

        st.subheader("Informations du projet")
        st.write(f"**Client :** {projet_client.get('client', 'Client')}")
