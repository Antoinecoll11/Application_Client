...
def charger_projet_client(tab_chargement):
    with tab_chargement:
        ...
        fichier_json = st.file_uploader(
            "Importer votre fichier projet",
            type=["json"]
        )

        if fichier_json is None:
            st.info("Veuillez importer votre fichier JSON projet.")
            return None

        projet_client = json.load(fichier_json)

        nom_fichier_json = fichier_json.name

        if st.session_state.get("dernier_fichier_json_client") != nom_fichier_json:
            st.session_state["json_client_deja_charge"] = False
            st.session_state["dernier_fichier_json_client"] = nom_fichier_json
        ...
        if "production_consommation_horaire" not in projet_client:
            st.error("Le fichier JSON importé ne contient pas les données horaires nécessaires.")
            st.write("Clés trouvées :", list(projet_client.keys()))
            return None
        ...
        cles_a_recharger = [
            ...
            "taux_ems",
            "aide_pv_active",
            "aide_batterie_active",
            "prix_electricite",
            "prix_injection",
            "prix_communaute_achat",
            "prix_communaute_vente",
            "cout_pv_par_wc",
            "cout_batterie_par_wh",
            "cout_ems",
            "subside_supplementaire",
            "taux_croissance_elec_annuel",
            "cout_om_pv_par_wc_an",
            "cout_om_batterie_par_wh_an",
            "perte_pv_annuelle",
        ...
        if not st.session_state["json_client_deja_charge"]:

            for cle in cles_a_recharger:
                if cle in projet_client:
                    st.session_state[cle] = projet_client[cle]

            if "coeffs_pac_mensuels" in projet_client:
                for i, val in enumerate(projet_client["coeffs_pac_mensuels"]):
                    st.session_state[f"coeff_pac_{i}"] = val

            st.session_state["json_client_deja_charge"] = True

        mon_tableau_base = pd.DataFrame(
            projet_client["production_consommation_horaire"]
        )

        mon_tableau_base["Date&Time"] = pd.to_datetime(mon_tableau_base["Date&Time"])
        mon_tableau_base["Inverter Output"] = pd.to_numeric(
            mon_tableau_base["Inverter Output"],
            errors="coerce"
        ).fillna(0)

        mon_tableau_base["Consumption_base"] = pd.to_numeric(
            mon_tableau_base["Consumption_base"],
            errors="coerce"
        ).fillna(0)

        st.success("Projet chargé avec succès.")

        st.subheader("Configuration de départ")
        st.write(f"**Puissance PV :** {projet_client.get('puissance_crete', 0)} kWc")
        st.write(f"**Batterie activée :** {'Oui' if projet_client.get('activer_batterie', False) else 'Non'}")
        st.write(f"**Modèle batterie :** {projet_client.get('choix_batterie', 'Non sélectionné')}")
        st.write(f"**EMS activé :** {'Oui' if projet_client.get('ems_actif', False) else 'Non'}")

        return projet_client, mon_tableau_base
...
def main():
    ...
    # 2. ONGLETS CLIENT
    # =========================================================
    tab_chargement, tab_saisons, tab_mensuel, tab_annuel, tab_budget, tab_finance = st.tabs([
        "Chargement projet",
        "Profils Journaliers",
        "Bilan Mensuel",
        "Résumé Annuel",
    ...
    # =========================================================
    # 3. CHARGEMENT DU JSON CLIENT
    # =========================================================
    donnees_client = charger_projet_client(tab_chargement)

    if donnees_client is None:
        st.stop()

    projet_client, mon_tableau_base = donnees_client

    data_import = {
        "puissance_crete": projet_client.get("puissance_crete", 0.0),
        "mode_prod": "Projet client JSON",
        "fichier_prod": None,
        "prod_specifique": 0.0,
        "df_repartition": None,
        "mode_conso": "Projet client JSON",
        "donnees_conso": None,
        "profil_choisi": "Consommation client",
    ...
    # =========================================================
    # 4. FONCTION POUR RECONSTRUIRE LE TABLEAU CLIENT
    # =========================================================
    def construire_tableau_client(activer_batterie=False, capa_wh=0.0, puiss_w=0.0):
        df = mon_tableau_base.copy()

        df["Date&Time"] = pd.to_datetime(df["Date&Time"])
        df["Inverter Output"] = pd.to_numeric(df["Inverter Output"], errors="coerce").fillna(0)
        df["Consumption_base"] = pd.to_numeric(df["Consumption_base"], errors="coerce").fillna(0)

        facteur_augmentation_prod = 1 + sidebar_data["augmentation_prod_pct"] / 100
        df["Inverter Output"] = df["Inverter Output"] * facteur_augmentation_prod

        if sidebar_data["borne_active"]:
            df["Conso_Borne"] = generer_profil_borne(
                df["Date&Time"],
                sidebar_data["puissance_borne_kw"],
                sidebar_data["horaires_borne"],
                sidebar_data["jours_selectionnes"]
            )
        else:
            df["Conso_Borne"] = 0.0
        ...
        if sidebar_data["chauffe_eau_actif"]:
            df["Conso_ChauffeEau"] = generer_profil_borne(
                df["Date&Time"],
                sidebar_data["puissance_chauffe_eau_kw"],
                sidebar_data["horaires_chauffe_eau"],
                sidebar_data["jours_chauffe_eau"]
            )
        else:
        ...
        if sidebar_data["pac_active"]:
            ...
            coeffs_pac_mensuels = sidebar_data.get(
                "coeffs_pac_mensuels",
                [1.40, 1.30, 1.10, 0.80, 0.40, 0.20, 0.10, 0.10, 0.30, 0.70, 1.10, 1.40]
            )

            coeffs_pac_dict = {
                mois: coeffs_pac_mensuels[mois - 1]
                for mois in range(1, 13)
            }

            coeff_pac = df["Date&Time"].dt.month.map(coeffs_pac_dict).fillna(1.0)
        ...
        if sidebar_data["chauffage_active"]:
            df["Conso_Chauffage"] = generer_profil_borne(
                df["Date&Time"],
                sidebar_data["puissance_chauffage_kw"],
                sidebar_data["horaires_chauffage"],
                sidebar_data["jours_chauffage"]
            )
        else:
            df["Conso_Chauffage"] = 0.0
        ...
        df["Autoconso_Directe"] = np.minimum(df["Inverter Output"], df["Consumption"])
        df["Autoconsommation"] = df["Autoconso_Directe"]
        df["Import_Reseau"] = np.maximum(0, df["Consumption"] - df["Inverter Output"])
        df["Export_Reseau"] = np.maximum(0, df["Inverter Output"] - df["Consumption"])

        if activer_batterie and capa_wh > 0 and puiss_w > 0:
            niveaux, charges, decharges, exports, achats = simuler_batterie(
                df["Inverter Output"].values,
                df["Consumption"].values,
                capacite_max_wh=capa_wh,
                puissance_max_w=puiss_w
            )

            df["Niveau_Batterie"] = niveaux
            df["Charge_Batterie"] = charges
            df["Decharge_Batterie"] = decharges
            df["Export_Reseau"] = exports
            df["Import_Reseau"] = achats
            df["Autoconsommation"] = df["Autoconso_Directe"] + df["Decharge_Batterie"]
        else:
            df["Niveau_Batterie"] = 0.0
            df["Charge_Batterie"] = 0.0
            df["Decharge_Batterie"] = 0.0
    ...
    # 5. SCÉNARIO PV SEUL
    # =========================================================
    mon_tableau_pv = construire_tableau_client(
    ...
    if scenario_batterie_disponible:
        mon_tableau_pv_batt = construire_tableau_client(
            activer_batterie=True,
            capa_wh=sidebar_data["capa_wh"],
            puiss_w=sidebar_data["puiss_w"]
        )

        capa_kwh_budget = sidebar_data.get("capa_kwh_commerciale", sidebar_data["capa_kwh"])
        capa_wh_budget = sidebar_data.get("capa_wh_commerciale", sidebar_data["capa_wh"])

        budget_pv_batt = calculer_budget(
            puissance_crete=data_import["puissance_crete"],
            activer_batterie=True,
            capa_kwh=capa_kwh_budget,
            capa_wh=capa_wh_budget,
            aide_pv_active=st.session_state["aide_pv_active"],
            aide_batterie_active=st.session_state["aide_batterie_active"]
        )
    ...
    if scenario_batterie_disponible:
        ...
        tableau_affichage = mon_tableau_pv
        budget_affichage = budget_pv
        indicateurs_affichage = calculer_indicateurs_annuels(
            mon_tableau_pv,
            0.0
        )
    ...
    if sidebar_data["ems_actif"] and scenario_batterie_disponible:
    ...
    if sidebar_data["puissance_reference_active"]:
        ...
        st.sidebar.caption(f"Heures en dépassement : {analyse_depassement['nb_heures_depassement']} h")
        st.sidebar.caption(f"Taux de dépassement : {analyse_depassement['taux_depassement']:.1f} %")
        st.sidebar.caption(f"Dépassement total : {analyse_depassement['depassement_total_kwh']:,.0f} kWh".replace(",", " "))
        st.sidebar.caption(f"Dépassement max : {analyse_depassement['depassement_max_kw']:.1f} kW")

    # =========================================================
    # 10. AFFICHAGE DES ONGLETS
    # =========================================================
